...
def get_host(url, headers, host_groups, fromTime):
    ...
    go = True
    while go:
        response = requests.get(host_url, headers=headers, params=init_params, verify=False)
        data = response.json()
        check_error(data)
        ...
        if(data.get('nextPageKey') == None):
            go = False
        time.sleep(0.500)

    return host_groups

def get_host_group(url, headers,fromTime):
    ...
    go = True
    while go:
        response = requests.get(host_group_url, headers=headers, params=init_params, verify=False)
        data = response.json()
        check_error(data)
        for entity in data['entities']:
...
def post_host_unit_analysis_metric(host_group_analysis_final, url, headers):
    ...
    lines = '\n'.join(metric_lines)
    response = requests.post(metric_url,headers=headers,data=lines.encode('utf-8'), verify=False)
    data = response.json()
    check_error(data)
    print(data)
...
def main():
    if len(sys.argv) != 6:
            print("Usage: python host_unit_analysis.py managed_url managed_api-token(API v2:Read Entities,API v1:Access problem and event feed, metrics, and topology) saas_url saas_api-token(API v2:Ingest metrics) fromTime")
            sys.exit(1)
    ...
    host_groups = get_host_group(urlGet, headersGet,fromTime)
    final_host_groups = get_host(urlGet, headersGet, host_groups,fromTime)
